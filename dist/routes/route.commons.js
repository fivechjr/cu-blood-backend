...
class Routes {
    ...
    bootstrap() {
        ...
        this.router.get('/insights/blood-types/:year', [
            ...
            //     group: ['User.bloodType']
            // })
            util_database_1.sequelize.query('SELECT users."bloodType", count(sessions.id) as count FROM users LEFT JOIN sessions on users.id = sessions."userId" GROUP BY users."bloodType" WHERE year(sessions."checkOut") = ?', { replacements: [req.params.year], type: util_database_1.sequelize.QueryTypes.SELECT }).then(d => {
