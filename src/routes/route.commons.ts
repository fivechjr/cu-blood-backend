...
class Routes {
    ...
    public bootstrap () : Router {
        ...
        this.router.get('/insights/blood-types/:year', [
            ...
            sequelize.query('SELECT users.bloodType, count(sessions.id) as Count FROM users LEFT JOIN sessions on users.id = sessions."userId" GROUP BY users."bloodType"', { type: sequelize.QueryTypes.SELECT}).then(d => {
